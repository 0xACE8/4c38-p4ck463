...
function index()
	if not nixio.fs.access("/etc/config/easytier") then
        ...
        entry({"admin", "services", "easytier"}, alias("admin", "services", "easytier", "easytier"),_("EasyTier"), 90).dependent = true
	entry({"admin", "services", "easytier", "easytier"}, cbi("easytier"),_("EasyTier"), 47).leaf = true
	entry({"admin", "services",  "easytier",  "easytier_log"}, form("easytier_log"),_("日志"), 48).leaf = true
